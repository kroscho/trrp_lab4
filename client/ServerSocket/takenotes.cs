...
namespace ServerSocket
{
    ...
    {
        int id, model_id, mark_id, price, cust_id, manag_id, manag_salary;
        ...
        public void WriteToNormalizedDb()
        {
            using (var conn = new MySqlConnection(MyDB.ConnString))
            {
                ...
                if ((long)sqlCommand.ExecuteScalar() == 0)
                {
                    strSql = "insert into dishes values(@id, @name, @mark, @price)";
                    sqlCommand = new MySqlCommand(strSql, conn);
                    sqlCommand.Parameters.AddWithValue("@id", model_id);
                ...
                if ((long)sqlCommand.ExecuteScalar() == 0)
                {
                    strSql = "insert into sales values(@id, @model, @customer, @manager)";
                    sqlCommand = new MySqlCommand(strSql, conn);
                    sqlCommand.Parameters.AddWithValue("@id", id);
                    sqlCommand.Parameters.AddWithValue("@model", model_id);
